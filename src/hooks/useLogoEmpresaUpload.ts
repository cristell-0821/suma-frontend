...
interface UseLogoEmpresaUploadReturn {
...
export const useLogoEmpresaUpload = (): UseLogoEmpresaUploadReturn => {
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>): Promise<string | null> => {
      ...
      try {
        const result = await uploadService.uploadLogoEmpresa(file);
        return result.url;
      } catch (err: any) {
        setUploadError(
          err.response?.data?.message || 'Error al subir el logo. Intenta de nuevo.'
        );
        return null;
      } finally {
